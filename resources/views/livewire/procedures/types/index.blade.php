...
<div class="py-12">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        @if (!$update)
            @can('create-type')
                @include('livewire.procedures.types.add')
            @endcan
        @else
            @can('create-type')
                @include('livewire.procedures.types.edit')
            @endcan
        @endif
                ...
                <div class="-my-2 overflow-x-auto sm:-mx-6 lg:-mx-8">
                    <div class="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
                        <div class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
                                ...
                                <div class="flex">
                                    ...
                                    <div class="ml-3 flex-1 md:flex md:justify-between">
                                        <p class="text-md text-blue-700">
                                            Listado de tipos de procesos en el sistema
                                        </p>
                                    </div>
                                </div>
                            </div>
                            ...
                            @if ($types->count())
                                @include('livewire.procedures.types.alerts')
                                ...
                                <table class="min-w-full divide-y divide-gray-200">
                                    <thead class="bg-gray-50">
                                        <tr>
                                            ...
                                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Monto Minimo</th>
                                            <th scope="col" colspan="2" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Monto Maximo</th>
                                        </tr>
                                    </thead>
                                        ...
                                        @foreach ($types as $type)
                                            <tr>
                                                ...
                                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                    @can('edit-type')
                                                        <button type="button" wire:click='edit({{  $type->id }})' class="inline-flex ml-6 items-center p-2 border border-transparent rounded-full shadow-sm text-white bg-blue-500 hover:bg-blue-600 focus:outline-none">
                                                            <!-- Heroicon name: outline/plus -->
                                                            <svg  class="h-6 w-6"xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                    ...
                                                    @endcan
                                                    @can('delete-type')
                                                        <button type="button" wire:click='destroy({{  $type->id }})' class="inline-flex ml-6 items-center p-2 border border-transparent rounded-full shadow-sm text-white bg-red-500 hover:bg-red-600 focus:outline-none">
                                                            <!-- Heroicon name: outline/plus -->
                                                            <svg class="h-6 w-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                                                            </svg>
